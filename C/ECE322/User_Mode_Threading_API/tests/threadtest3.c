...
void timer_handler (int signum) 
{
	...
	{
		printf("threadtest3.c: --------->$$$$ interrupting\n");
		threadYield();
	}
} 
...
{
	int param = *((int*)arg);
	printf("threadtest3.c: t1 started %d\n", param);
	...
	*result = param;
	...
	printf ("threadtest3.c: finished with math! (%d)\n", *result);
	
	sleep(1);
	printf("threadtest3.c: t1 - done result=%d\n", *result);
	return result;
}
...
int main(void)
{
        printf("threadtest3.c: pre-emptive testing\n");
	...
	printf("threadtest3.c: created thread 1.\n");	
	
	id2 = threadCreate(t1,(void*)&p2);
	printf("threadtest3.c: created thread 2.\n");

	threadJoin(id1, (void*)&result1);
	printf("threadtest3.c: joined #1 --> %d.\n",*result1);

	threadJoin(id2, (void*)&result2);
	printf("threadtest3.c: joined #2 --> %d.\n",*result2);

        printf("threadtest3.c: main process returning...\n");
	interruptsAreDisabled = TRUE; // Don't want to interrupt in the middle of a return
        return 0;
}
