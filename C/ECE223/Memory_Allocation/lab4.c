 ...
 *
 * Assumptions: Provided code runs correctly and the free list generated by mem.c
 * will always contain a dummy block of size zero.
 *
...
int main(int argc, char **argv)
{
    ...
    printf("Welcome to MP4: Dynamic Memory Allocation\n");
    ...
    {
        printf("\n----- Begin unit test driver 0: Provided Driver -----\n");
        char *string;
    ...
    {
        printf("\n----- Begin unit test driver 1: Provided Driver -----\n");
        ...
        */
        ...
        // depending on your implementation
        int init_size_free_list = (4096/sizeof(chunk_t))-1;
        ...
        // for example, if your chunk_t is 8 and 511 units on free list
    ...
    {
        printf("\n----- Begin unit test driver 2: Empty List Testing -----\n");
        ...
        Mem_free(p2);
        Mem_free(p3);
        printf("list should be full again\n");
        Mem_print();
        Mem_stats();
    ...
    {
        printf("\n----- Begin unit test driver 3: Whole Page Testing -----\n");
        int *p1;
        int num_bytes_1;

        // allocate whole page (including header)
        num_bytes_1 = 4096 - (int) sizeof(chunk_t);
        p1 = (int *) Mem_alloc(num_bytes_1);
        printf("first allocation of %d bytes p=%p \n", num_bytes_1, p1);
        printf("should see %d bytes free\n", (4096/(int)sizeof(chunk_t))-1);
        Mem_print();
    ...
    {
        printf("\n----- Begin unit test driver 4: Gap Testing -----\n");

        // unit_size is 8 or 16 depending on your chunk_t size
        int unit_size = sizeof(chunk_t);
        // init_size_free_list might be 512, 511, 256, or 255 
        // depending on your implementation
        int init_size_free_list = (4096/sizeof(chunk_t))-1; // The -1 is the dummy header
        ...
        // for example, if your chunk_t is 8 and 511 units on free list
        // then request  511 - (99+1) - (49+1) -1 = 360 units
        num_bytes_3 = init_size_free_list - num_bytes_1/unit_size - num_bytes_2/unit_size - 3;
        num_bytes_3 *= unit_size;
        p3 = (int *) Mem_alloc(num_bytes_3);
        printf("third allocation of %d bytes p=%p\n", num_bytes_3, p3);
       
        // Free blocks 1 and 3 to create a gap in memory
        printf("blocks 1 and 3 freed - should be a gap in free memory\n");
        Mem_free(p1);
        Mem_free(p3);
        ...
        printf("p2 = %p\n", p2);
        Mem_free(p2);
        printf("block 2 freed - gap should be gone\n");
        Mem_print();
        Mem_stats();

        printf("\n----- End unit test driver 4 -----\n");
    }
    else if(!dprms.EquilibriumTest) 
    {
        printf("Invalid unit driver specified. Specify -u [0-4] or -e for equilibrium.\n");
    } 
...
void equilibriumDriver(driver_params *ep)
{
    ...
    for (i = 0; i < ep->WarmUp; i++) {
        ...
        list_insert(mem_list, (data_t *) ptr, 0);
        ptr = NULL;
    }
    printf("After warmup\n");
    if (!ep->SysMalloc) {
    ...
    for (i = 0; i < pos; i++) {
        ptr = (int *) list_remove(mem_list, 0);
        assert(ptr != NULL);
        size = -ptr[0];
        assert(min_num_ints <= size && size <= ep->AvgNumInts+ep->RangeInts);
        for (index = 1; index < size; index++)
...
void getCommandLine(int argc, char **argv, driver_params *ep)
{
    /* The getopt function creates three global variables:
    ...
    while ((c = getopt(argc, argv, "w:t:s:a:r:f:h:u:codve")) != -1)
        switch(c) {
...
void printCommandLine(void)
{
    ...
    printf("Note: See lab4.c and mem.c for documentation on each flag\n\n");
