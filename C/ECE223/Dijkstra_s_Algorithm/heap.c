...
PriorityQueue *PQ_Initialize(int num_vertices)
{
    PriorityQueue *PQ;
...
void PQ_Insert(PQItem Item, PriorityQueue *PQ)
{   
    ...
    if (PQ_Full(PQ)) {    // could improve to first check if found
...
PQItem PQ_Remove(PriorityQueue *PQ)
{
    ...
    if (PQ_Empty(PQ))        /* result is undefined if PQ was empty */
    {
